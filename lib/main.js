...
module.exports = {
  ...
  activate() {
    ...
    this.tmpfile = new File(this.tmpobj.name)
  ...
  lint(editor, fix=false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.tmpfile.write(editorText).then(() => {
        let args = [
          'check',
          this.tmpobj.name,
          '--quiet',
  ...
  formatter(mode) {
    ...
    for (let selection of selections) {
      ...
      this.tmpfile.write(selectionText).then(() => {
        let args = [
          'format',
          this.tmpobj.name,
          '--quiet',
        ...
        execFile(this.executable, args, opts, (error, stdout, stderr) => {
          if (stderr) {
            ...
            this.tmpfile.read().then((newText) => {
              if (mode) {
