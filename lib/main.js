...
module.exports = {
  ...
  lint(editor, fix = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let args = [...this.ruffExtraArgs, "check", "--quiet", "--output-format=json", `--stdin-filename=${editorPath}`];
      ...
      const child = execFile(this.ruffExe, args, opts, (error, stdout, stderr) => {
        if (error && error.code === "ENOENT") {
          atom.notifications.addError(`\`${this.ruffExe}\` not found.`, { description: `Check the "Ruff Command" setting in linter-ruff.` });
          return resolve([]);
        }
        if (stderr) {
          reject(error);
